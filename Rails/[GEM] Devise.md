...
Devise gem은 회원가입/인증/권한설정 등을 구현하기 위한 대표적인 rails gem이다.
...
1. devise 젬에 회원가입 안내 메일을 보내기 위한 작업

	`config/environments/development.rb`에 아래를 추가
...
```bash
$ rails g devise User
Running via Spring preloader in process 28762
      invoke  active_record
      create    db/migrate/20160812053210_devise_create_users.rb
...
User 모델에 custom field를 추가한다면 [여기](http://jacopretorius.net/2014/03/adding-custom-fields-to-your-devise-user-model-in-rails-4.html) 블로그를 참고하자!
...
> `config/initializers/devise.rb`에서 devise와 관련된 다양한 정보들을 설정할 수 있다.
...
```
> method 명은 임의로 정해도 된다.
> 같은 `provider`, `uid`를 가진 회원을 찾고, 없다면 새로 만들어 반환한다.
> 이때 이메일과 이름, 프로필 등의 유저 데이터는 페이스북 정보를 이용하고 비밀번호는 랜덤으로 생성한다.
> name, email, image 같은 정보는 user model에 속성이 존재할 때 사용하면 된다. 

### 5) callback URL을 처리할 Controller 추가

페이스북 인증이 성공적으로 이루어 졌을 시 불러질 콜백 메서드를 만들어야 한다!
...
```
> `request.env["omniauth.auth"]`는 페이스북 정보 데이터이다.
> 유저 정보를 방금 User Model에서 정의한 method를 이용해 불러온다.
유저가 유효하면 로그인.
그렇지 않으면 회원가입 폼으로 이동.

## 오늘의 집의 SNS 로그인
오늘의 집에서 페이스북, 카카오톡으로부터 사용자 프로필 이미지 정보를 요구한다.
